...
const AddShows = () => {
  const { image_base_url, fetchDashboardData } = useAdminAuth();
  ...
  const fetchNowPlayingMovies = async (retryCount = 0) => {
    try {
      ...
      if (retryCount < 1) {
        ...
        setTimeout(() => fetchNowPlayingMovies(retryCount + 1), 1000);
        return;
      }
      setNowPlayingMovies([]);
    } finally {
  ...
  const handleAddShow = async () => {
    try {
      if (
        ...
        toast.error("Please select a movie, set a price, and add show times");
        return;
      }

      // backendUrl is already defined above
      ...
      const response = await axios.post(`${backendUrl}/api/show/add`, payload, { withCredentials: true });

      if (response.data.success) {
        toast.success("Shows added successfully");
        // Refresh dashboard data to show new shows immediately
